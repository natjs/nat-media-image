...
- (void)viewDidLoad {
    ...
    if ([_style isEqual:@"dots"]) {
        _pageControl = [[UIPageControl alloc] initWithFrame:CGRectMake(0, self.view.frame.size.height-60, self.view.frame.size.width, 20)];
        _pageControl.numberOfPages = self.numberOfPhotos;
        _pageControl.currentPageIndicatorTintColor = [UIColor whiteColor];
        _pageControl.pageIndicatorTintColor = [UIColor grayColor];
        [self.view addSubview:_pageControl];

    }else if ([_style isEqual:@"label"]){
        _pageLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, self.view.frame.size.height-70, self.view.frame.size.width, 20)];
